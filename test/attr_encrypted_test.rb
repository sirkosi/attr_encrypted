...
class AttrEncryptedTest < Minitest::Test
  ...
  def test_should_store_email_in_encrypted_attributes
    assert User.encrypted_attrs.include?(:email)
  end

  def test_should_not_store_salt_in_encrypted_attributes
    refute User.encrypted_attrs.include?(:salt)
  end
  ...
  def test_attr_encrypted_should_not_use_the_same_attribute_name_for_two_attributes_in_the_same_line
    refute_equal User.encrypted_attrs[:email][:attribute], User.encrypted_attrs[:without_encoding][:attribute]
  end
  ...
  def test_should_decrypt_email_when_reading
    ...
    options = @user.encrypted_attrs[:email]
  ...
  def test_should_inherit_encrypted_attributes
    assert_equal [User.encrypted_attrs.keys, :testing].flatten.collect { |key| key.to_s }.sort, Admin.encrypted_attrs.keys.collect { |key| key.to_s }.sort
  end
  ...
  def test_should_not_inherit_unrelated_attributes
    ...
    assert SomeOtherClass.encrypted_attrs.empty?
  end
  ...
  def test_should_work_with_aliased_attr_encryptor
    assert User.encrypted_attrs.include?(:aliased)
  end
  ...
  def test_should_specify_the_default_algorithm
    assert YetAnotherClass.encrypted_attrs[:email][:algorithm]
    assert_equal YetAnotherClass.encrypted_attrs[:email][:algorithm], 'aes-256-gcm'
  end
  ...
  def test_encrypted_attributes_state_is_not_shared
    ...
    assert_equal :encrypting, user.encrypted_attrs[:ssn][:operation]
    assert_nil another_user.encrypted_attrs[:ssn][:operation]
  end
